...
function dump_power_state()
{
	...
	cat /sys/power/rpmh_stats/master_stats | awk -F "[:]" '{if ($2=="0") {if (NR=="39") print "CDSP"} \
	if ($2=="0") {if (NR=="31") print "ADSP_ISLAND"} if ($2=="0") {if (NR=="23") print "ADSP" } \
	if ($2=="0") {if (NR=="15") print "MPSS"} if ($2=="0") {if (NR=="7") print "APPS"}}' >> $LOGFILE
